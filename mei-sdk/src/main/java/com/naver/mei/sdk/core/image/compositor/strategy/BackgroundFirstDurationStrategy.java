...
public class BackgroundFirstDurationStrategy extends LongestDurationStrategy {
	@Override
	public int calculate(CompositionElement background, List<AnimatedElement> animatedElements) {
		if (background instanceof AnimatedElement) {
			return ((AnimatedElement) background).getDuration();
		}

		return super.calculate(background, animatedElements);
	}
}